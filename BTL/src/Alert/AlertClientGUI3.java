...
public class AlertClientGUI3 extends JFrame {
    ...
    public AlertClientGUI3(String groupAddress, int port) {
        super("Weather Alert Client 3");
        try {
        ...
        // Log File
        try {
            logWriter = new PrintWriter(new FileWriter("weather_alerts3.log", true)); // Sửa tên file log cho client 3
            System.out.println("Log file weather_alerts3.log opened successfully.");
        } catch (IOException e) {
        ...
        if (SystemTray.isSupported()) {
            ...
            trayIcon = new TrayIcon(image, "Weather Alert Client 3");
    ...
    public static void main(String[] args) throws Exception {
        ...
        int port = (args.length > 1) ? Integer.parseInt(args[1]) : 4448;
        if (args.length < 2) {
            System.out.println("Warning: Using default values - group: 239.255.0.1, port: 4448. Correct syntax: java Alert.AlertClientGUI3 <groupAddress> <port>");
        }
        SwingUtilities.invokeLater(() -> {
            new AlertClientGUI3(groupAddress, port).setVisible(true);
        });
    }
}