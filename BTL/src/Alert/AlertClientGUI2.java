...
public class AlertClientGUI2 extends JFrame {
    ...
    public AlertClientGUI2(String groupAddress, int port) {
        super("Weather Alert Client 2");
        try {
        ...
        // Log File
        try {
            logWriter = new PrintWriter(new FileWriter("weather_alerts2.log", true)); // Sửa tên file log cho client 2
            System.out.println("Log file weather_alerts2.log opened successfully.");
        } catch (IOException e) {
        ...
        if (SystemTray.isSupported()) {
            ...
            trayIcon = new TrayIcon(image, "Weather Alert Client 2");
    ...
    public static void main(String[] args) throws Exception {
        ...
        int port = (args.length > 1) ? Integer.parseInt(args[1]) : 4447;
        if (args.length < 2) {
            System.out.println("Warning: Using default values - group: 239.255.0.1, port: 4447. Correct syntax: java Alert.AlertClientGUI2 <groupAddress> <port>");
        }
        SwingUtilities.invokeLater(() -> {
            new AlertClientGUI2(groupAddress, port).setVisible(true);
        });
    }
}